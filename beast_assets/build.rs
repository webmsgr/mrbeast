...
fn main() {
    ...
    let video_file_path = data_path.join("f%04d.png");
    let audio_file_path = data_path.join("audio.ogg");
    let frames_rs = data_path.join("frames.rs");
    let args = ["-i", "source.mp4", "-vf", "scale=-1:480", video_file_path.as_os_str().to_str().unwrap(), "-i", "source.mp4", "-vn", audio_file_path.as_os_str().to_str().unwrap()];
    ...
    let frames: Vec<_> = glob(data_path.join("*.png").as_os_str().to_str().unwrap()).expect("failed to read pattern").filter_map(|e| e.ok()).collect();
